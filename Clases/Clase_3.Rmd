...
Ejercicio
Seleccionen la talla (lght) y peso de todos los bebes con un peso a nacer menor a 2500g (biwt), solo los primeros 10 resultados
```{r}
...
genere una variable indicadora para la variable clin ifentificado los pacientes que se les tomo muestra (sample o yes) que sean de etiopia. Cuantos son?
...
ari %>% 
  mutate(
    ...
    country == "Ethiopia" & wght >= 250 & wght <= 3800 ~ "et_peso_norm"
  )
)
```

ejercicio case when
determinar los bebes de papa nueva guinea que tengan un peso mayor a 3000 y no se hicieron la prueba clinica o la negaron (clin).
